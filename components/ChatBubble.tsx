...
export default function ChatBubble({ user }: ChatBubbleProps) {
  ...
  return (
    <>
      {open ? (
        <div className="fixed bottom-4 right-4 z-50 h-[28rem] w-[22rem] max-w-[calc(100vw-1rem)] overflow-hidden rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] shadow-2xl">
          ...
          {error ? (
            <p className="px-3 py-2 text-xs text-rose-700 dark:text-rose-300">{error}</p>
          ) : null}

          <div className="flex h-[calc(100%-3rem)]">
            <div className="w-2/5 border-r border-[color:var(--border)] p-2">
              <select
                value={newTargetUid}
              ...
              <div className="mt-2 space-y-1">
                {loading ? (
                  ...
                  conversations.map((conversation) => {
                    ...
                    const label = friend?.displayName ?? friend?.email ?? targetUid ?? "Unknown";
                    const unread =
