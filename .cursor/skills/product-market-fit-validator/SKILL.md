...
4. Decide whether the feature should be:
   - kept as is
   - repositioned
   - narrowed
...
When reviewing product fit, return:
...
Keep findings concrete and tied to existing surfaces, docs, or UI entry points.
...
- If a feature overlaps with `Review mistakes`, `Practice`, `Leaderboard`, `PvP`, or `Social`, compare the user job directly instead of judging each in isolation.

## Guardrails

Do not:
- Approve a feature just because it is technically implementable
- Create parallel surfaces with nearly identical user jobs
- Treat naming problems as cosmetic when the real issue is product overlap
- Expand scope before clarifying the feature's role in the product
